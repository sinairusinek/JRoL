    ...
    dataReady = function () {//This is called as soon as data is loaded
        ...
        a.iterNodes(
            function (b) { //This is where we populate the array used for the group select box
                ...
                // alert(b.x);
                a.clusters[b.color];
                a.clusters[b.color] || (a.clusters[b.color] = []); // TODO: Declare a dict with a "nodes"gfkrjfjceiddlrejdrndruellirfhrtj
...
function configSigmaElements(config) {
    ...
    var a = [],
        b, x = 1;
    for (b in sigInst.clusters) a.push('<div style="line-height:12px"><a href="#' + b + '"><div style="width:40px;height:12px;border:1px solid #fff;background:' + b + ';display:inline-block"></div> ' + (x++) + ' (' + sigInst.clusters[b].length + ' nodes)</a></div>');
    //a.sort();
